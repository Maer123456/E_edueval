...
class BaichuanGenerator(ModelGenerator):
    """
    Generator class for Baichuan models (Baichuan-7B, Baichuan2-13B, etc.)
    """
    ...
    def model_init(self):
        ...
        try:
            ...
            # Keep settings adjusted for Baichuan models
            model_kwargs = {
    ...
    def format_prompt(self, prompt):
        """Format prompt for Baichuan models using chat format"""
        # For Baichuan models, it's better to format correctly for the model type
        ...
        # If model doesn't have chat template, check for known Baichuan models
        if "baichuan" in self.model_name.lower():
            # Baichuan models use this format
            return f"<用户>{prompt}</用户><AI助手>"
    ...
    def generate_output(self, tokenizer, model, batch_size=1, max_items=None, offset=0):
        """
        Override generate_output to handle Baichuan-specific prompt formatting
            ...
            try:
                if self.is_vllm:
                    ...
                    for j, prompt in enumerate(formatted_prompts):
                        ...
                        # Remove the prompt part from the output - Baichuan specific handling
                        if "baichuan" in self.model_name.lower():
                            # For Baichuan models, response is after the prompt format
                            if "</用户><AI助手>" in output_text:
                                output_text = output_text.split("</用户><AI助手>", 1)[1].strip()
                            elif "<AI助手>" in output_text:
                                output_text = output_text.split("<AI助手>", 1)[1].strip()
    ...
    def extract_choice_answer(self, text):
        """Improved choice answer extraction for Baichuan models"""
        # First try parent class method
        answer = super().extract_choice_answer(text)
        if answer and answer in "ABCD":
            return answer
            
        # Try Baichuan-specific extraction if parent method failed
        cleaned_text = text.strip()
        
        # Check for standalone letter at beginning of response
        first_paragraph = cleaned_text.split('\n')[0].strip()
        if len(first_paragraph) <= 5:
            choices = re.findall(r'[A-Da-d]', first_paragraph)
            if choices:
                return choices[0].upper()
            
        # Try to find the answer after specific markers in Chinese
        ...
        # Look for patterns like "I choose X" or "The answer is X"
        patterns = [
            r'我选择\s*([A-Da-d])',
            r'选择\s*([A-Da-d])',
            r'答案是\s*([A-Da-d])',
