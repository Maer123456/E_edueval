...
class GLMGenerator(ModelGenerator):
    """
    Generator class for GLM models (ChatGLM, GLM-3, etc.)
    """
    ...
    def model_init(self):
        ...
        try:
            ...
            # Keep settings adjusted for GLM models
            model_kwargs = {
    ...
    def format_prompt(self, prompt):
        """Format prompt for GLM models using chat format"""
        # For ChatGLM models, it's better to format correctly for the model type
        ...
        # If model doesn't have chat template, check for known GLM models
    ...
    def generate_output(self, tokenizer, model, batch_size=1, max_items=None, offset=0):
        """
        Override generate_output to handle GLM-specific prompt formatting
            ...
            try:
                if self.is_vllm:
                    ...
                    for j, prompt in enumerate(formatted_prompts):
                        ...
                        # Remove the prompt part from the output - GLM specific handling
                        if "chatglm" in self.model_name.lower():
                            # For ChatGLM models, the response is after the prompt format
                            if "答：" in output_text:
                                output_text = output_text.split("答：", 1)[1].strip()
    ...
    def extract_choice_answer(self, text):
        """Improved choice answer extraction for GLM models"""
        # First try parent class method
        answer = super().extract_choice_answer(text)
        if answer and answer in "ABCD":
            return answer
            
        # Try GLM-specific extraction if parent method failed
        cleaned_text = text.strip()
        
        # Check if the answer is at the beginning (GLM often puts the answer at the start)
        ...
        # As a last resort, check for answer pattern "Answer: X" or just single letter response
        if len(cleaned_text) <= 5:
            choices = re.findall(r'[A-Da-d]', cleaned_text)
            if choices:
                return choices[0].upper()
                
        if "answer" in cleaned_text.lower():
            answer_part = cleaned_text.lower().split("answer", 1)[1]
            choices = re.findall(r'[a-d]', answer_part)
            if choices:
                return choices[0].upper()
